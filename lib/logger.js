...
function formatLog(fmt, colorful, level, type, pos, msgs, rawMsg) {
  var msg = rawMsg ? msgs : formatMsg.apply(null, msgs);
  var pid = process.pid;
...
Logger.prototype = {
  _log: function (type, msgs, pos) {
    ...
    pos = pos || getPos();
    if (pos.indexOf(this._root) === 0) {
      pos = pos.substr(this._root.length);
    }
    this._stream.write(
      formatLog(
        this.fmt, 
